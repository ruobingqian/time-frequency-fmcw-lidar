...
%% For the hand video, read one frame at a time; for other datasets, only one frame is included (set frame_num=1);
frame_num=1;
data_pre=data(:,:,frame_num);

%% remove invalid pts (index generated from the insight laser SW) in each interferogram 
for i=1:size(data_pre,2)
test=data_pre(:,i);
range = 57; % the optimized range for this dataset
data_valid(:,i) = remove_invalid_pt(data_pre(:,i)); 
end
...
%depth_map=170-(max_loc_final+50)*s_factor; % for the mannequin head data, a different ref arm position was set, therefore a global shift is applied.
imagesc(depth_map);colormap jet;b=imagesc(depth_map); set(b,'AlphaData',~isnan(depth_map))
caxis([0 95]); %%colorscale for hand data
%caxis([40 160]);%%colorscale for mannequin head data
